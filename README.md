...
其中建议这样分层：
- `rules.md`：内容规则，例如标题、正文、语气、标签、图片风格约束
- `profile.json`：运营配置，例如 `reviewBeforePublish`、`imageMode`、`publishPath`、账号定位、受众、目标
- `scripts/`：执行逻辑，例如生成资产、选图、卡片生成、浏览器自动发布
...
- 不去水印，也不提供去水印指导
- 自动学习是参考模式，不会偷偷改主配置
- 默认全自动发布，只有在配置 `reviewBeforePublish: true` 时才停在审核态
